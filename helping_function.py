"""
This module contains all the necessary helper functions for the process.
"""
import os  # For file paths
import boto3  # For performing operations on the AWS S3 bucket.
import zipfile  # For Unzipping the files
import requests  # For downloading files from a given URL.
import pandas as pd  # For creating a CSV file from the given data.
...
def download(url, download_path, filename):
    ...
    """
    file = ""
    log.info("Downloading the xml file by the link provided.")
    try:
        # Retrieving the information stored in the file.
        ...
        # Verifying if the requests received correct response.
        if response.ok:
            # If the directories specified in the download path do not exist, create them.
